...
// ✅ Script Otomatis
$onLoginScript = <<<RSC
:put (",ntf,,enable,"); {:local date [ /system clock get date ];:local year [ :pick \$date 7 11 ];:local month [ :pick \$date 0 3 ];:local comment [ /ip hotspot user get [/ip hotspot user find where name="\$user"] comment]; :local ucode [:pic \$comment 0 2]; :if (\$comment = "") do={ /sys sch add name="\$user" disable=no start-date=\$date interval="1d"; :delay 2s; :local exp [ /sys sch get [ /sys sch find where name="\$user" ] next-run]; :local getxp [len \$exp]; :if (\$getxp = 15) do={ :local d [:pic \$exp 0 6]; :local t [:pic \$exp 7 16]; :local s ("/"); :local exp ("\$d\$s\$year \$t"); /ip hotspot user set comment=("exp:" . \$exp) [find where name="\$user"];}; :if (\$getxp = 8) do={ /ip hotspot user set comment="\$date \$exp" [find where name="\$user"];}; :if (\$getxp > 15) do={ /ip hotspot user set comment=\$exp [find where name="\$user"];}; /sys sch remove [find where name="\$user"]; [:local mac $"mac-address"; /ip hotspot user set mac-address=\$mac [find where name=\$user]]}}
RSC;
...
$addProfile->setArgument('limit-uptime', $limit_uptime);
...
$schedRequest->setArgument('interval', $session_timeout);
