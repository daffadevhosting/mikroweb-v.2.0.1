...
// Ambil input body JSON
$input = json_decode(file_get_contents('php://input'), true);
$profileName = $input['name'] ?? '';

if (!$profileName) {
    echo json_encode(["success" => false, "error" => "Nama profile tidak diberikan"]);
    exit;
}
...
try {
    ...
    // Cari `.id` dari profile yang mau dihapus
    $client->sendSync(new RouterOS\Request('/ip/hotspot/user/profile/remove', ['.id' => $keyId]));
